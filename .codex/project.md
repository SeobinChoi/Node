...
- The first-session public-demo tutorial auto-runs one `/api/demo/generate` request using the exact built-in `meetingSummary` synthetic catalog fixture, waits for its truthful Gemini/fallback result, and never triggers persistence, export, or download actions.
